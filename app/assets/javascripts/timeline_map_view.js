(function (M, $) {
  ...
  TimelineMapView.prototype = {
    ...
    drawLocation: function drawLocation(line, depth) {
      ...
      while (p) {
        ...
        if (p.next == null || p.next.getColumn() != topLeft.x) {
          ...
          drawLabel(this.labelCtx, line.thing.name, corner.clone().add(size.x / 2, 6));
          this.timelineCtx.globalCompositeOperation = 'source-over';
          drawInsetBox(this.timelineCtx, corner, size, 5, color);
          topLeft = null;
        }
        p = p.next;
      }
    ...
    drawObject: function drawObject(line) {
      ...
      this.timelineCtx.strokeStyle = (line.thing.color || M.draw.Color.black).toCss();
  ...
  function drawLabel(ctx, text, loc) {
